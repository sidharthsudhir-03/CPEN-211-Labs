...
installed and then open Terminal and change directory to the folder contining
this file and then type "make".

If using Windows install cygwin using the instructions in cygwinlab7.pdf.
Then, run Cygwin by clicking on the cygwin icon.  If installed correctly,
running cygwin will open up a UNIX style command line environment or terminal.
The rest of the commands below should be typed in the cygwin terminal.  Next,
Copy the entire contents of this folder to C:\cygwin64\home\<username>, where
<username> is your windows username. This directory should already exist.  If
you install the 32-bit version of cygwin it will be C:\cygwin\home\<username>.
To build assembler type "make".  You should see:
